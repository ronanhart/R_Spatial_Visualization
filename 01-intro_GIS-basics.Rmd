...
knitr::include_graphics("Plots/pictures/spheroid.png")
```
...
knitr::include_graphics("Plots/pictures/wgs.png")
```
...
knitr::include_graphics("Plots/pictures/local.png")
```
...
```{r projectFig, fig.cap = "Conceptual demonstration of map projections", fig.align = 'center', out.width = '50%', echo = F}
knitr::include_graphics("Plots/pictures/projection.png")
```
...
knitr::include_graphics("Plots/pictures/utm-1024x512.jpg")
```

```{r utmUSA, fig.cap = "UTM for the US", fig.align = 'center', out.width = '50%', echo = F}
knitr::include_graphics("Plots/pictures/utm_usa.png")
```
...
```{r features, echo = F, fig.cap = "Figure demonstrating points (red), polylines (black), and polygon (blue)", fig.align = 'center'}

set.seed(1)

plygn <- matrix(runif(8), ncol = 2)
plygn[4,] <- plygn[1,]
plygn <- sp::Polygon(plygn)
plygn <- sp::Polygons(list(plygn), ID = "area")
plygn <- sp::SpatialPolygons(list(plygn))

ln <- sp::Line(matrix(runif(10, min(sp::bbox(plygn), max(sp::bbox(plygn)))), ncol = 2))
ln <- sp::Lines(list(ln), ID = c("ln1")) 
ln <- sp::SpatialLines(list(ln))

pts <- sp::SpatialPoints(matrix(runif(10, min(sp::bbox(plygn), max(sp::bbox(plygn)))), ncol = 2))
...
```{r utahExample, echo = F, fig.cap = "Figure demonstrating points (major Utah cities), polylines (major Utah highways), and polygons (shape of Utah boundary)", fig.align = 'center'}
...
ut_city <- maps::us.cities %>%
  dplyr::filter(country.etc == "UT") %>%
  sf::st_as_sf(coords = c("long", "lat"), crs = 4326)

interstate <- st_read("Data/Examples/utah_interstate", "utah_interstate", quiet = T) %>%
...
```{r contDemo, echo = F, fig.cap = "Map showing a raster with continuous values (elevation)", fig.align = 'center'}
dem <- rast("Data/Examples/elevation.tif")
plot(dem, col = hcl.colors(100, "Grays"), main = "Elevation")
```

```{r discDemo, echo = F, fig.cap = "Map showing a raster with discrete values (land cover)", fig.align = 'center'}
land <- rast("Data/demo/landcover_demo.tif")
land_summary <- read.csv("Data/demo/landcover_summary.csv") 
land %>%
  as.data.frame(xy = T) %>%
  rename(Value = landcover_demo) %>%
  # join with the summary table
  left_join(land_summary, by = "Value") %>%
  # turn into a factor (so they'll be displayed in this given order)
  mutate(Landcover = factor(
    Landcover, levels = c("Open Water", "Polar & High Montane Scrub, Grassland & Barrens", 
                          "Recently Disturbed or Modified", "Developed & Other Human Use", 
                          "Open Rock Vegetation", "Nonvascular & Sparse Vascular Rock Vegetation", 
                          "Shrub & Herb Vegetation", "Forest & Woodland", "Desert & Semi-Desert"))) %>%
  ggplot(aes(x = x, y = y, fill = Landcover)) + 
  geom_raster() +
  scale_fill_manual(values = c("#4a6fa3", "#e9f2fa", "#aa1e23", "#dc9881", 
                               "#b3afa5", "#bdcc93", "#6cab67", "#1d6634", "#d1bb82"),
                    labels = c("Open Water", "Polar & High Montane Scrub, Grassland & Barrens", 
                               "Recently Disturbed or Modified",
                               "Developed & Other Human Use", "Open Rock Vegetation",
                               "Nonvascular & Sparse Vascular Rock Vegetation", "Shrub & Herb Vegetation", 
                               "Forest & Woodland", "Desert & Semi-Desert")) +
...
```{r landsat, echo = F}
landsat <- rast("Data/demo/landsat_demo.tif") 
names(landsat) <- c("Red", "Green", "Blue")
print(landsat)
```

```{r landsatDemo, echo = F, fig.cap = "Plotting a single band of a satellite image will only show the individual values of RGB", fig.align = 'center'}
par(mfcol = c(1,3))
plot(landsat$Red, col = hcl.colors(100, "Reds 3"), legend = F)
plot(landsat$Green, col = hcl.colors(100, "Greens 3"), legend = F)
plot(landsat$Blue, col = hcl.colors(100, "Blues 3"), legend = F)
```

But these 3 bands come together to make a true-color image. 

```{r landsatDemoFull, echo = F, fig.cap = "True-color satellite image of the Salt Lake region", fig.align = 'center'}
par(mfcol = c(1,1))
plotRGB(landsat*255*4)
```
...
```{r fine, echo = F, fig.cap = "Map showing a raster with fine resolution (30m X 30m)", fig.align = 'center'}
plot(dem, col = hcl.colors(100, "Grays"), main = "Elevation (30mX30m)")
```

Compared to the same raster but with a coarser resolution (in this case 300 m X 300 m)

```{r coarse, echo = F, fig.cap = "Map showing a raster with coarse resolution (300 m X 300 m)", fig.align = 'center'}
dem_coarse <- aggregate(dem, fact = 10)
plot(dem_coarse, col = hcl.colors(100, "Grays"), main = "Elevation (300mX300m)")
...
2. The finer the resolution, the more cells there are, and so the time, computation power, and disk space to do any sort of computation or analysis on these cells increases.
